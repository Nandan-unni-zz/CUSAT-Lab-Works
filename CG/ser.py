...
class Car:
    ...
    def get_rotated_points(self, vertices):
        ...
        for x, y in vertices:
            points.append(
                [
                    round(x * cos(radians(self.angle)) - y * sin(radians(self.angle))),
                    round(x * sin(radians(self.angle)) + y * cos(radians(self.angle))),
                ]
            )
        return points
    ...
    def display(self):
        x = self.start_point[0] * cos(radians(self.angle)) - self.start_point[1] * sin(
            radians(self.angle)
        )
        y = self.start_point[0] * sin(radians(self.angle)) + self.start_point[1] * cos(
            radians(self.angle)
        )
        self.create_line()
        self.draw_car(x, y)
